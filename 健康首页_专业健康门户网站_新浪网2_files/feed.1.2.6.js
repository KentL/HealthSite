/* [2,717,248] published at 2015-01-28 14:48:37 */
...
var feedTemplate2 = (function(){
  ...
  html.push('<div class="feed-card-scroll-w" data-id="feedCardListScrollNews">');
  ...
  html.push('<span class="feed-card-scroll-num" data-id="feedCardListScrollNewsNum">1/<%= news.sub.length %></span><a href="javascript:;" class="feed-card-scroll-prev feed-card-icon" data-id="feedCardListScrollNewsPrev" suda-uatrack="key=index_feed&value=sns_prev">Prev</a><a href="javascript:;" class="feed-card-scroll-next feed-card-icon" data-id="feedCardListScrollNewsNext" suda-uatrack="key=index_feed&value=sns_next">Next</a>');
  html.push('</div>');
...
var userData = (function(){
  ...
  function getUserDataFromNet(cateID, tagID, callback){
    IO.jsonp({
      url: 'http://user.mix.sina.com.cn/subscribe/api/subscribe/get',
      data: {
        from: 'pc',
        project_id: [cateID, tagID].join(',')
      },
      onsuccess: function(result) {
        if(result && result.result && result.result.status && result.result.status.code == 0){
          var keywords = [];
          var lids = [];
          var lidObjs = result.result[cateID];
          if(lidObjs) {
            for(var i = 0; i < lidObjs.length; i ++) {
              lids.push(win.parseInt(lidObjs[i].c, 10));
            }
          }
          var keyObjs = result.result[tagID];
          if(keyObjs) {
            for(var i = 0; i < keyObjs.length; i ++) {
              keywords.push(keyObjs[i].c);
            }
          }
          callback({
            lids: lids,
...
var FeedCard = (function(){
  ...
  FeedCard.prototype = {
    ...
    _createScrollCard: function(){
      var that = this;
      var elems = dom.getByAttributeName(that.container, 'data-id', 'feedCardListScrollNews', 'div');
      if(elems.length == 0){
        return;
      }
      ...
      var feedCardListScrollNewsNum = dom.getByAttributeName(that.container, 'data-id', 'feedCardListScrollNewsNum', 'span')[0];
      scrollCard.onChange = function(index, length){
        feedCardListScrollNewsNum.innerHTML = (index + 1) + '/' + length;
      };
      dom.addEvent(dom.getByAttributeName(that.container, 'data-id', 'feedCardListScrollNewsPrev', 'a')[0], 'click', function(e){

        scrollCard.goToPrev();

        dom.preventDefault(e);
      });
      dom.addEvent(dom.getByAttributeName(that.container, 'data-id', 'feedCardListScrollNewsNext', 'a')[0], 'click', function(e){
...
var COLUMN_EDITOR_ACTION = 'http://user.mix.sina.com.cn/subscribe/api/subscribe/replace';
...
var Tab = (function(){
  ...
  var TabClass = function(obj){
    ...
    that.columnEditorForm = dom.$('columnEditorForm');
    that.columnEditorFormHTML = that.columnEditorForm.innerHTML;
  ...
  TabClass.prototype = {
    ...
    uploadData: function(hideToast){
      ...
      that.columnEditorForm.action = COLUMN_EDITOR_ACTION + '?callback=SINAFEEDCARD_COLUMNS_EDIT_DONE1';
      that.columnEditorForm.innerHTML = that.columnEditorFormHTML + that.generateFormHTML(that.tabs, that.subCateID);
      that.columnEditorForm.submit();
    },
    uploadKeywords: function() {
      this.columnEditorForm.action = COLUMN_EDITOR_ACTION + '?callback=SINAFEEDCARD_COLUMNS_EDIT_DONE';
      this.columnEditorForm.innerHTML = this.columnEditorFormHTML + this.generateFormHTML(this.keywords, this.subTagID);
      this.columnEditorForm.submit();
    },
    generateFormHTML: function(ary, projID) {
      var html = [];
      for(var i = 0; i < ary.length; i ++) {
        html.push('<input type="hidden" name="data[' + i + '][c]" value="' + ary[i] + '" />');
        html.push('<input type="hidden" name="data[' + i + '][w]" value="' + i + '" />');
      }
      html.push('<input type="hidden" name="project_id" value="' + projID + '" />');
      return html.join('');
    },
    getSubData: function() {
      var that = this;
      userData.getUserDataFromNet(that.subCateID, that.subTagID, function(data){
        // 如果获取成功
        var tabs = win.FEED_CARD_INFO.tabs || [];
        var keywords = win.FEED_CARD_INFO.keywords || [];
        if(data){
          if(data.lids && data.lids.length){
...
;(function(){
  ...
  win.SINAFEEDCARD_COLUMNS_EDIT_DONE1 = function(result) {
    tab.uploadKeywords();
  };
